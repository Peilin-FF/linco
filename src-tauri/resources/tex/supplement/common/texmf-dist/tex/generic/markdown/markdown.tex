...
\seq_new:N \g__markdown_experimental_lua_options_seq
...
\cs_generate_variant:Nn
  ...
  { Vnnn }
\cs_new_protected:Nn
  \__markdown_add_experimental_lua_option:n
  {
    \__markdown_add_experimental_option:Vn
      \c__markdown_option_layer_lua_tl
      { #1 }
  }
\cs_new_protected:Nn
  \__markdown_add_experimental_option:nn
  {
    \seq_gput_right:cn
      { g__markdown_ #1 _options_seq }
      { #2 }
...
\cs_new_protected:Nn \__markdown_typecheck_option:n
  {
...
\cs_new_protected:Nn \__markdown_option_tl_to_csname:nN
  {
    \tl_set:Nn
      \l_tmpa_tl
      { \str_uppercase:n { #1 } }
...
\regex_const:Nn
  \c__markdown_option_regex
  { ^markdownOption }
\prg_new_protected_conditional:Nnn
  \__markdown_csname_to_option_str:nN
  { T }
  {
...
\cs_new_protected:Nn \__markdown_with_various_cases:nn
  {
    \seq_clear:N
      \l_tmpa_seq
    \seq_map_inline:Nn
...
\cs_new_protected:Nn \__markdown_camel_case:N
  {
...
\cs_new_protected:Nn \__markdown_snake_case:N
  {
    \regex_replace_all:nnN
      { ([a-z])([A-Z]) }
...
\__markdown_add_lua_option:nnn
  ...
  { true }
\ExplSyntaxOff
\def\markdownLastModified{2026-06-17}%
\def\markdownVersion{3.16.0-0-g4d0fb773}%
...
\seq_new:N \g__markdown_experimental_plain_tex_options_seq
\seq_gput_right:NV
  \g__markdown_option_layers_seq
  \c__markdown_option_layer_plain_tex_tl
\cs_new_protected:Nn
...
\cs_new_protected:Nn
  \__markdown_prepend_option_value:nn
  {
    \__markdown_get_option_value:nN
      { #1 }
      \l_tmpa_clist
...
\cs_new_protected:Nn
  \__markdown_append_option_value:nn
  {
    \__markdown_get_option_value:nN
      { #1 }
      \l_tmpa_clist
...
\cs_new_protected:Nn
  \__markdown_setup_snippet:nn
  {
    ...
    \__markdown_resolve_snippet_name:nN
      { #1 }
      \l_tmpa_tl
...
\seq_gput_right:Nn
  \g__markdown_renderers_seq
  { inputBlockHtmlProcessingInstruction }
\prop_gput:Nnn
  \g__markdown_renderer_arities_prop
  { inputBlockHtmlProcessingInstruction }
  { 1 }
...
\seq_gput_right:Nn
  \g__markdown_renderers_seq
  { inlineHtmlProcessingInstruction }
\prop_gput:Nnn
  \g__markdown_renderer_arities_prop
  { inlineHtmlProcessingInstruction }
  { 1 }
...
\prop_gput:Nnn
  ...
  { jekyllDataEmpty }
  { 1 }
\cs_new_protected:Nn \__markdown_define_renderers:
  {
    \seq_map_inline:Nn
      \g__markdown_renderers_seq
      {
...
\cs_new_protected:Nn \__markdown_define_renderer:n
  {
...
\cs_new_protected:Nn \__markdown_renderer_tl_to_csname:nN
  {
    \tl_set:Nn
      \l_tmpa_tl
      { \str_uppercase:n { #1 } }
...
\bool_new:N
  \g__markdown_prepending_renderer_bool
\bool_new:N
  \g__markdown_appending_renderer_bool
\bool_new:N
  \g__markdown_unprotected_renderer_bool
\cs_new_protected:Nn \__markdown_define_renderer:nNn
  {
    \keys_define:nn
...
\regex_const:Nn
  \c__markdown_prepending_key_regex
  { \^$ }
\regex_const:Nn
  \c__markdown_appending_key_regex
  { [\$+]$ }
\regex_const:Nn
  \c__markdown_prepending_or_appending_key_regex
  { [\^\$+]$ }
\keys_define:nn
  { markdown/options/renderers }
  {
...
\cs_new_protected:Nn \__markdown_define_renderer_prototypes:
  {
    \seq_map_inline:Nn
      \g__markdown_renderers_seq
      {
...
\cs_new_protected:Nn \__markdown_define_renderer_prototype:n
  {
...
\cs_new_protected:Nn \__markdown_renderer_prototype_tl_to_csname:nN
  {
    \tl_set:Nn
      \l_tmpa_tl
      { \str_uppercase:n { #1 } }
...
\bool_new:N
  \g__markdown_prepending_renderer_prototype_bool
\bool_new:N
  \g__markdown_appending_renderer_prototype_bool
\bool_new:N
  \g__markdown_unprotected_renderer_prototype_bool
\cs_new_protected:Nn \__markdown_define_renderer_prototype:nNn
  {
    \keys_define:nn
...
\msg_new:nnn
  ...
  { pin-theme-version }
  {
    Write~"#1@#2"~to~pin~version~"#2"~of~the~theme~"#1".
    This~will~keep~your~documents~from~suddenly~breaking
    when~we~have~released~future~versions~of~the~theme
    with~backwards-incompatible~syntax~and~behavior.
  }
\msg_new:nnnn
  { markdown }
  { unknown-theme-version }
...
\prop_gput:Nnn
  ...
  { witiko / diagrams / v2 }
  {
    ...
    \tl_set:Nn
      ...
      { pdf }
    \tl_new:N
      \l__markdown_diagrams_command_tl
    \__markdown_setup:n
      {
        rendererPrototypes = {
          fencedCodeAttributeContextBegin = {
    ...
    \cs_set:Nn
      \__markdown_diagrams_infostrings_current:n
      {
        ...
        \__markdown_diagrams_render_diagram:nnVn
          { #1 }
          { #1.pdf }
          \l_tmpb_tl
          { Graphviz~image }
    ...
    \prop_gput:NnV
      ...
      \l_tmpa_tl
    \tl_new:N
      \g__markdown_diagrams_mmdc_command_tl
    \tl_gset:Nn
      \g__markdown_diagrams_mmdc_command_tl
      {
        \tl_if_empty:NTF
    ...
    \cs_set:Nn
      \__markdown_diagrams_infostrings_current:n
      {
        ...
        \__markdown_diagrams_render_diagram:nVVn
          { #1 }
          \l_tmpa_tl
          \l_tmpb_tl
          { PlantUML~image }
...
\cs_new_protected:Nn \__markdown_format_lua_options:
  {
    \tl_gclear:N
...
\cs_new_protected:Nn \__markdown_format_lua_option:n
  {
    \__markdown_typecheck_option:n
      { #1 }
    \__markdown_get_option_type:nN
...
\cs_new_protected:Npn
  \markinline
  {
    ...
    \peek_after:Nw
      \__markdown_inline_read_markdown_text:
  }
\cs_new_protected:Nn
  \__markdown_inline_read_markdown_text:
  {
