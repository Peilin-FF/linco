...
#[tauri::command]
pub async fn fs_list_dir(path: String, host: Option<String>) -> Result<Vec<DirEntry>, String> {
...
#[tauri::command]
pub async fn fs_read_file(path: String, host: Option<String>) -> Result<String, String> {
    crate::blocking::run(move || {
        if let Some(h) = host.filter(|s| !s.is_empty()) {
...
#[tauri::command]
pub async fn fs_read_bytes(path: String, host: Option<String>) -> Result<String, String> {
    crate::blocking::run(move || {
        use base64::Engine;
        if let Some(h) = host.filter(|s| !s.is_empty()) {
...
#[tauri::command]
pub async fn fs_write_bytes(path: String, b64: String, host: Option<String>) -> Result<(), String> {
    crate::blocking::run(move || {
        use base64::Engine;
