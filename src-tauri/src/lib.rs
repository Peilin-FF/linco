...
pub fn run_with_context(context: tauri::Context<tauri::Wry>) {
    tauri::Builder::default()
        ...
        .invoke_handler(|invoke| {
            // Remote notes and sign-in popups never get Linco's filesystem, PTY,
            // credentials, or other custom commands. Plugin ACLs are main-only too.
            if invoke.message.webview_ref().label() != "main" {
                invoke
                    .resolver
                    .reject("Remote browser views cannot invoke Linco commands");
                return true;
            }
            let handler: fn(tauri::ipc::Invoke<tauri::Wry>) -> bool = tauri::generate_handler![
                ...
                notion_mcp::notion_connection,
                notion_mcp::notion_tool,
                terminal::term_start,
                terminal::term_write,
