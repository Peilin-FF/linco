...
/// 是否为 git 仓库。
#[tauri::command]
pub fn git_is_repo(repo: String, host: Option<String>) -> bool {
    git(&host, &repo, &["rev-parse", "--is-inside-work-tree"])
        .map(|s| s.trim() == "true")
        .unwrap_or(false)
}
...
#[tauri::command]
pub fn git_status(repo: String, host: Option<String>) -> Result<GitStatus, String> {
    if !git_is_repo(repo.clone(), host.clone()) {
        return Ok(GitStatus {
            is_repo: false,
            branch: String::new(),
            ahead: 0,
...
#[tauri::command]
pub fn git_stage(repo: String, path: String, host: Option<String>) -> Result<(), String> {
    git(&host, &repo, &["add", "--", &path]).map(|_| ())
}

#[tauri::command]
pub fn git_unstage(repo: String, path: String, host: Option<String>) -> Result<(), String> {
    git(&host, &repo, &["restore", "--staged", "--", &path]).map(|_| ())
}

#[tauri::command]
pub fn git_stage_all(repo: String, host: Option<String>) -> Result<(), String> {
    git(&host, &repo, &["add", "-A"]).map(|_| ())
}

#[tauri::command]
pub fn git_unstage_all(repo: String, host: Option<String>) -> Result<(), String> {
    git(&host, &repo, &["reset"]).map(|_| ())
}
...
#[tauri::command]
pub fn git_commit(repo: String, message: String, host: Option<String>) -> Result<String, String> {
...
#[tauri::command]
pub fn git_pull(repo: String, host: Option<String>) -> Result<String, String> {
    git(&host, &repo, &["pull", "--ff-only"])
}

#[tauri::command]
pub fn git_push(repo: String, host: Option<String>) -> Result<String, String> {
    git(&host, &repo, &["push"])
}

#[tauri::command]
pub fn git_fetch(repo: String, host: Option<String>) -> Result<String, String> {
    git(&host, &repo, &["fetch", "--all", "--prune"])
}
...
#[tauri::command]
pub fn git_branches(repo: String, host: Option<String>) -> Result<Vec<GitBranch>, String> {
    let raw = git(
        &host,
        &repo,
        &[
...
#[tauri::command]
pub fn git_checkout(repo: String, branch: String, host: Option<String>) -> Result<String, String> {
    git(&host, &repo, &["checkout", &branch])
}

#[tauri::command]
pub fn git_create_branch(repo: String, name: String, host: Option<String>) -> Result<String, String> {
    git(&host, &repo, &["checkout", "-b", &name])
}
...
/// 某次提交的 diff(用于历史查看)。
#[tauri::command]
pub fn git_show(repo: String, hash: String, host: Option<String>) -> Result<String, String> {
    git(&host, &repo, &["show", "--no-color", &hash])
}
...
#[tauri::command]
pub fn git_stash_list(repo: String, host: Option<String>) -> Result<Vec<GitStash>, String> {
...
#[tauri::command]
pub fn git_stash_push(repo: String, message: String, host: Option<String>) -> Result<String, String> {
    if message.trim().is_empty() {
...
#[tauri::command]
pub fn git_stash_apply(repo: String, index: u32, host: Option<String>) -> Result<String, String> {
    git(&host, &repo, &["stash", "apply", &format!("stash@{{{index}}}")])
}

#[tauri::command]
pub fn git_stash_pop(repo: String, index: u32, host: Option<String>) -> Result<String, String> {
    git(&host, &repo, &["stash", "pop", &format!("stash@{{{index}}}")])
}

#[tauri::command]
pub fn git_stash_drop(repo: String, index: u32, host: Option<String>) -> Result<String, String> {
    git(&host, &repo, &["stash", "drop", &format!("stash@{{{index}}}")])
}
